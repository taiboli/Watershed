...
compute_auc_across_dimensions <- function(number_of_dimensions, watershed_posteriors, gam_posteriors, real_valued_outliers_test1, binary_outliers_test2) {
	...
	neg_list <- c()
  	...
  	for (dimension in 1:number_of_dimensions) {
		...
		evaROC <-	
		 list(
		real_valued_outliers_test1=real_valued_outliers_test1,
		watershed_posteriors=watershed_posteriors,
		test_outlier_status=test_outlier_status,
		watershed_sens=watershed_roc_obj$curve[,2],
...
evaluate_watershed_shell <- function(input_file, number_of_dimensions, model_name, pseudoc, lambda_init, output_stem, n2_pair_pvalue_fraction, binary_pvalue_threshold, lambda_costs, nfolds, vi_step_size, vi_threshold) {
  ...
  this.order = order(N2_pairs)
  feat_tmp = feat_all[this.order,]
  this.n2pair <- N2_pairs[this.order]
  discrete_outliers_tmp = discrete_outliers_all[this.order,]
  binary_outliers_tmp = binary_outliers_all[this.order,]
  fraction_binary_outliers_tmp = fraction_binary_outliers_all[this.order,]
  
  this.test.index <- !is.na(this.n2pair)
  ...
  if (number_of_dimensions == 1) {
    ...
    feat_test <- rbind(feat_tmp[this.test.index,][seq(from=1,to=sum(this.test.index),by=2),], 
                       feat_tmp[this.test.index,][seq(from=2,to=sum(this.test.index),by=2),])
    discrete_outliers_test1 <- as.matrix(c(discrete_outliers_tmp[this.test.index][seq(from=1,to=sum(this.test.index),by=2)], 
                                           discrete_outliers_tmp[this.test.index][seq(from=2,to=sum(this.test.index),by=2)]))
    discrete_outliers_test2 <- as.matrix(c(discrete_outliers_tmp[this.test.index][seq(from=2,to=sum(this.test.index),by=2)], 
                                           discrete_outliers_tmp[this.test.index][seq(from=1,to=sum(this.test.index),by=2)]))
    binary_outliers_test1 <- as.matrix(c(binary_outliers_tmp[this.test.index][seq(from=1,to=sum(this.test.index),by=2)], 
                                         binary_outliers_tmp[this.test.index][seq(from=2,to=sum(this.test.index),by=2)]))
    binary_outliers_test2 <- as.matrix(c(fraction_binary_outliers_tmp[this.test.index][seq(from=2,to=sum(this.test.index),by=2)], 
                                         fraction_binary_outliers_tmp[this.test.index][seq(from=1,to=sum(this.test.index),by=2)]))
    # Absolute pvalues from test prediction data set (to be used for RNA-only analysis)
    real_valued_outliers_test1 <- -log10(abs(as.matrix(c(data_input$outlier_pvalues[this.order,][this.test.index][seq(from=1,to=sum(this.test.index),by=2)], 
                                                         data_input$outlier_pvalues[this.order,][this.test.index][seq(from=2,to=sum(this.test.index),by=2)]))) + 1e-7)
    
  } else {
    # Extraction of test data for Watershed
    feat_test <- rbind(feat_all[this.test.index,][seq(from=1,to=sum(this.test.index),by=2),], feat_all[this.test.index,][seq(from=2,to=sum(this.test.index),by=2),])
    discrete_outliers_test1 <- rbind(discrete_outliers_tmp[this.test.index,][seq(from=1,to=sum(this.test.index),by=2),], 
                                     discrete_outliers_tmp[this.test.index,][seq(from=2,to=sum(this.test.index),by=2),])
    discrete_outliers_test2 <- rbind(discrete_outliers_tmp[this.test.index,][seq(from=2,to=sum(this.test.index),by=2),], 
                                     discrete_outliers_tmp[this.test.index,][seq(from=1,to=sum(this.test.index),by=2),])
    binary_outliers_test1 <- rbind(binary_outliers_tmp[this.test.index,][seq(from=1,to=sum(this.test.index),by=2),], 
                                   binary_outliers_tmp[this.test.index,][seq(from=2,to=sum(this.test.index),by=2),])
    binary_outliers_test2 <- rbind(fraction_binary_outliers_tmp[this.test.index,][seq(from=2,to=sum(this.test.index),by=2),],
                                   fraction_binary_outliers_tmp[this.test.index,][seq(from=1,to=sum(this.test.index),by=2),])
    # Absolute pvalues from test prediction data set (to be used for RNA-only analysis)
    real_valued_outliers_test1 <- -log10(abs(rbind(data_input$outlier_pvalues[this.order,][this.test.index,][seq(from=1,to=sum(this.test.index),by=2),], 
                                                   data_input$outlier_pvalues[this.order,][this.test.index,][seq(from=2,to=sum(this.test.index),by=2),])) + 1e-7)
  }
...
#######################################
## Evaluate using existing models
#######################################
evaluate_watershed_existingModel <- function(input_file, number_of_dimensions, model_name, pseudoc, lambda_init, 
                                             output_stem, n2_pair_pvalue_fraction, binary_pvalue_threshold, 
                                             lambda_costs, nfolds, vi_step_size, vi_threshold,
                                             watershed_model) {
  ########################
  ## Load in data
  ########################
  ...
  gam_data <- watershed_model$gam_model_params
  # Compute GAM Predictions on test data in CPP file ("independent_crf_exact_updates.cpp")
  gam_posterior_test_obj <- update_independent_marginal_probabilities_exact_inference_cpp(feat_test, binary_outliers_test1, gam_data$gam_parameters$theta_singleton, gam_data$gam_parameters$theta_pair, gam_data$gam_parameters$theta, matrix(0,2,2), matrix(0,2,2), number_of_dimensions, choose(number_of_dimensions, 2), FALSE)
  gam_test_posteriors <- gam_posterior_test_obj$probability
  ...
  ## Compute Watershed Posterior probabilities for held-out test data (ie the N2 pairs)
  #######################################
  posterior_info_test <- update_marginal_posterior_probabilities(feat_test, discrete_outliers_test1, watershed_model$model_params)
  posterior_prob_test <- posterior_info_test$probability  # Marginal posteriors
  posterior_pairwise_prob_test <- posterior_info_test$probability_pairwise  # Pairwise posteriors
...
arguments <- parse_args(OptionParser(usage = "%prog [options]", description="Watershed command line args",
	option_list=list(
		...
		make_option(c("-t","--training_model"), default = NULL, help="The Watershed model (evaluation object)"),
...
training_model_file <- arguments$training_model
...
## Train model on non-N2 pairs and evaluate model on N2-pairs
#######################################
evaluation_model <- readRDS(training_model_file)

evaluation_object <- evaluate_watershed_existingModel(input_file, number_of_dimensions, model_name, pseudoc, lambda_init, output_stem, n2_pair_pvalue_fraction, binary_pvalue_threshold, lambda_costs, nfolds, vi_step_size, vi_threshold, evaluation_model)
