...
learn_watershed_model_parameters_from_training_data <- function(training_input_file, number_of_dimensions, model_name, pseudoc, lambda_init, binary_pvalue_threshold, lambda_costs, nfolds, vi_step_size, vi_threshold) {
	...
	## Load in data
	########################
	data_input <- load_watershed_data(training_input_file, number_of_dimensions, n2_pair_pvalue_fraction, binary_pvalue_threshold)
...
predict_watershed_posteriors <- function(watershed_object, prediction_input_file, number_dimensions) {
	...
	########################
	prediction_data_input <- load_watershed_data(prediction_input_file, number_of_dimensions, n2_pair_pvalue_fraction, binary_pvalue_threshold)
	# Parse data_input for relevent data
...
arguments <- parse_args(OptionParser(usage = "%prog [options]", description="Watershed command line args",
	option_list=list(
		...
		make_option(c("-l","--l2_prior_parameter"), default="NA", help="Parameter defining L2 (gaussian) distribution the acts as a prior on the parameters of the conditional random Field (the model parameters defining Z|G"),
...
#watershed_object <- readRDS( paste0(output_stem, "_prediction_object.rds"))

evaluation_model <- readRDS(training_model_file)
watershed_object <- list(mean_feat = evaluation_model$mean_feat, sd_feat = evaluation_model$sd_feat, model_params = evaluation_model$model_params)

# if(!file.exists(paste0(output_stem, "_prediction_object.rds"))){
#   tmp <- readRDS(paste0(output_stem, "_evaluation_model.rds"))
#   data_input <- load_watershed_data(training_input_file, number_of_dimensions, n2_pair_pvalue_fraction, binary_pvalue_threshold)
#   # Parse data_input for relevent data
#   feat_all <- data_input$feat
#   #######################################
...
#   mean_feat <- apply(feat_all, 2, mean)
#   sd_feat <- apply(feat_all, 2, sd)
#   watershed_object <- list(mean_feat=mean_feat,sd_feat=sd_feat, model_params=tmp)
# } else{
#   watershed_object <- readRDS( paste0(output_stem, "_prediction_object.rds"))
# }
